...
// tag: merge sort, segment tree, binary indexed tree
public class CountOfSmallerNumbersAfterSelf {
    ...
    public static void main(String[] args) {
        ...
        List<Integer> output = test.countSmaller(nums); // [2, 1, 1, 0]
        Utils.printIntergerArray(output);

        // Example 2
        nums = new int[]{-1};
        output = test.countSmaller(nums); // [0]
        Utils.printIntergerArray(output);

        // Example 3
        nums = new int[]{-1, -1};
        output = test.countSmaller(nums); // [0, 0]
        Utils.printIntergerArray(output);
    }
}
